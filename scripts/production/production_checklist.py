...
class ProductionChecker:
    ...
    async def check_environment_config(self):
        ...
        is_prod_db = not any(x in db_url.lower() for x in ['localhost', '127.0.0.1', 'sqlite'])
        self.add_check(
            "生产数据库配置",
            is_prod_db,
            weight=3,
        ...
        is_prod_redis = not any(x in redis_url.lower() for x in ['localhost', '127.0.0.1'])
        self.add_check(
            "生产Redis配置",
            is_prod_redis,
            weight=2,
            message="应使用远程Redis集群"
        )
        ...
        try:
            ...
            async with db_manager.get_async_session() as session:
                result = await session.execute(
                    text("SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'matches'")
                )
            ...
            self.add_check(
                ...
                table_exists,
                weight=2,
                message="核心数据表存在" if table_exists else "需要执行 make data-setup"
            )
    ...
    async def check_monitoring_setup(self):
        ...
        grafana_dashboard = Path("monitoring/grafana/dashboards/data_platform_dashboard.json")
        self.add_check(
            "Grafana监控面板",
        ...
        self.add_check(
            ...
            prom_accessible,
            weight=2,
            message="Prometheus可访问" if prom_accessible else "需要启动: docker-compose up prometheus"
        )
    ...
    async def generate_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "critical_issues": [c for c in self.checks if not c["status"] and c["weight"] >= 3],
                "warnings": [c for c in self.checks if not c["status"] and c["weight"] < 3]
            }
        }
    ...
    def print_report(self, report: dict):
        ...
        categories = {
            "🔧 环境配置": [c for c in report['checks'] if any(x in c['name'] for x in ['密钥', '配置', '环境'])],
            "📡 服务连接": [c for c in report['checks'] if any(x in c['name'] for x in ['连接', 'API', '数据库', 'Redis'])],
            "📈 监控告警": [c for c in report['checks'] if any(x in c['name'] for x in ['监控', 'Prometheus', 'Grafana'])],
            "🛡️ 安全设置": [c for c in report['checks'] if any(x in c['name'] for x in ['安全', '调试', 'JWT', 'CORS'])]
        }
        ...
        if report['overall_score'] >= 75:
            ...
            print("  1. 🔧 修复上述关键配置问题")
            print("  2. 🧪 重新运行检查: python scripts/production/production_checklist.py")

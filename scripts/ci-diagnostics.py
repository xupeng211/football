        ...
        for name, check_func in checks:
            ...
            try:
                ...
                results[name] = {
                    "success": False,
                    "error": str(e),
                    "issues": [{"severity": "error", "message": f"{name}失败: {e}"}],
                }
                        ...
                        issues.append(
                            {
                                "severity": "critical",
                                "message": (
                                    f"requirements.lock缺少关键依赖: {missing_deps}"
                                ),
                                "solution": (
    ...
    def _generate_report(self, results: Dict) -> Dict:
        ...
        if critical_issues:
            ...
            for issue in critical_issues[:3]:  # 只显示前3个最重要的
                recommendations.append(f"  - {issue['message']}: {issue['solution']}")
        ...
        if any("格式" in i["message"] or "ruff" in i["message"] for i in self.issues):
            quick_fixes.append("poetry run ruff check --fix .")
        if any("类型" in i["message"] or "mypy" in i["message"] for i in self.issues):

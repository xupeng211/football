...
class RealDataAnalyzer:
    ...
    def load_data(self):
        ...
        for row in cursor.fetchall():
            match = {
                "id": row[0], "api_id": row[1], "league_id": row[2],
                "league_name": row[3], "season": row[4], "matchday": row[5],
                "status": row[6], "utc_date": row[7], "home_team_id": row[8],
                "home_team_name": row[9], "away_team_id": row[10],
                "away_team_name": row[11], "home_score": row[12],
                "away_score": row[13], "result": row[14]
            }
        ...
        for row in cursor.fetchall():
            team = {
                "id": row[0], "api_id": row[1], "name": row[2],
                "short_name": row[3], "crest": row[4], "founded": row[5],
                "venue": row[6], "league_id": row[7]
            }
            teams.append(team)
    ...
    def analyze_league_characteristics(self, matches):
        ...
        league_stats = defaultdict(lambda: {
            'matches': 0, 'total_goals': 0, 'home_wins': 0,
            'draws': 0, 'away_wins': 0, 'high_scoring': 0,
            'low_scoring': 0, 'home_goals': 0, 'away_goals': 0
        })
        ...
        for league, stats in league_stats.items():
            ...
            analysis_results[league] = {
                ...
                "攻防特点": self.classify_league_style(avg_goals, home_win_rate, high_scoring_rate)
            }
    ...
    def analyze_recent_trends(self, matches):
        ...
        for league, trends in league_trends.items():
            if trends['matches'] > 0:
                ...
                print(f"📊 {league}: {trends['matches']}场, 场均{avg_goals:.2f}球, 主场{home_rate:.1%}")
    ...
    def find_interesting_patterns(self, matches):
        ...
        patterns['最高比分'] = {
            ...
            "总进球": total_goals,
            "日期": highest_scoring['utc_date'][:10] if highest_scoring['utc_date'] else "未知"
        }
        ...
        max_draw_league = max(league_draws.items(),
        ...
        for league, stats in league_draws.items():
            home_wins = sum(1 for m in matches if m['league_name'] == league and m['result'] == 'H')
            if stats['total'] > 0:
        ...
        # 显示发现
        print("🎯 有趣发现:")
        print(f"  🥅 最高比分: {patterns['最高比分']['比赛']} ({patterns['最高比分']['联赛']})")
        print(f"  ⚖️ 平局最多: {patterns['平局最多联赛']['联赛']} ({patterns['平局最多联赛']['平局率']})")
        print(f"  🏠 主场最强: {patterns['主场优势最强']['联赛']} ({patterns['主场优势最强']['主场胜率']})")

        return patterns

    def generate_comprehensive_report(self, matches, teams, league_analysis, trends, patterns):
        """生成综合分析报告"""
        ...
        comprehensive_report = {
            ...
            "关键发现": patterns,
            "数据质量评估": {
                "完整比赛比例": len([m for m in matches if m['home_score'] is not None]) / len(matches),
                "数据新鲜度": "实时API数据",
                "覆盖完整性": "6个主要联赛100%覆盖"
            },
            "预测模型建议": {
                ...
                "模型类型": ["逻辑回归 (基线)", "随机森林 (特征重要性)", "XGBoost (最终模型)"],
                "验证策略": "时间序列划分，避免数据泄露"
            }
        }
    ...
    def run_full_analysis(self):
        ...
        # 生成综合报告
        report = self.generate_comprehensive_report(matches, teams, league_analysis, trends, patterns)

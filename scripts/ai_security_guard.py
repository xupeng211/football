...
class AISecurityGuard:
    ...
    def _check_forbidden_imports(self, content: str, file_path: str,
                                lines: list[str]) -> list[SecurityIssue]:
        """检查禁止的导入"""
        issues = []
        forbidden_imports = (
    ...
    def _check_code_patterns(self, content: str, file_path: str,
                           lines: list[str]) -> list[SecurityIssue]:
        """检查代码模式"""
        issues = []
        code_patterns = (
    ...
    def _check_file_contents(self, content: str, file_path: str,
                           lines: list[str]) -> list[SecurityIssue]:
        """检查文件内容"""
        issues = []
        forbidden_contents = (
                    ...
                    for target in node.targets:
                        if (isinstance(target, ast.Name) and
                            ...
                            if (isinstance(node.value, ast.Constant) and
                                ...
                                if len(node.value.value) < 32:
                                    auto_fix = (
                                        f"{target.id} = "
                                        f"os.environ.get('{target.id.upper()}')"
                                    )
                                    issue = SecurityIssue(
                                        severity="HIGH",
    ...
    def _generate_secret_fix(self, forbidden: str, line: str) -> str:
        ...
        return (
            f"# 使用环境变量: os.environ.get('SECRET_KEY')  "
            f"# 生成的密钥: {new_secret}"
        )
    ...
    def _generate_text_report(self) -> str:
        ...
        for severity in severity_order:
            if severity in by_severity:
                ...
                report.append(f"{severity_icons[severity]} {severity} 级别问题 ({len(issues)}个)")
                report.append("-" * 30)
